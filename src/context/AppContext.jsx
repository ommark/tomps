...
export function AppProvider({ children }) {
    ...
    const [showWelcomeScreen, setShowWelcomeScreen] = useState(false); // New state for the welcome screen
    ...
    useEffect(() => {
        ...
        const canInitialize = isAuthReady && userId && isNewUser && !settingsError && predefinedActivities?.length > 0;

        if (canInitialize && !initializationLock.current) {
            initializationLock.current = true;
            console.log("Running one-time setup for new user...");

            // Show the welcome screen for this new user
    ...
    // New function to close the welcome screen
    ...
    const value = {
        ...
        showWelcomeScreen, dismissWelcomeScreen, // Add new state and function to context
    };
